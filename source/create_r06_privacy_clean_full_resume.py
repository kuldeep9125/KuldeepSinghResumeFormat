...
OUTPUT_ROOT = ROOT / "outputs" / "R06_Privacy_Clean_Full_Resume"
REPO_ROOT = ROOT / "work" / "KuldeepSinghResumeFormat"
REPO_R06 = REPO_ROOT / "resumes" / "R06"
LOCAL_ROOT = Path("/Users/kuldeepsingh/Downloads/RESUME/R06_Privacy_Clean_Full_Resume")

VERSION = "R06"
FILE_BASE = "Kuldeep_Singh_Privacy_Clean_EV_Software_Technical_Lead_Resume_R06"
...
PROJECT_SCAN_NOTES = [
    "# R06 Privacy-Clean Project Notes",
    "",
    "## Source Reviewed",
    "",
    "Source reviewed: user-provided hybrid-control Simulink project archive.",
    "",
    "The archive had damaged/incomplete central-directory metadata, but streaming extraction and local-header scans showed a real hybrid-control project with Simulink, MATLAB, DBC, MF4, generated target, calibration, and log artifacts.",
    "",
    "## Resume-Safe Evidence Extracted Without Project/File Names",
    "",
    "- Final Simulink model revisions and harness metadata.",
    "- Stateflow logic, block-diagram XML, model workspace, and code dictionary artifacts.",
    "- MATLAB communication/control scripts covering J1939, torque/speed control, BMS charging, battery interfaces, and telematics.",
    "- DBC evidence with 26 CAN messages and 211 signals across BMS, VCU command, motor status, telematics, EV faults, remote access, charge, precharge, voltage/current, torque/speed, and diagnostics.",
    "- Build/calibration evidence: generated MOT firmware outputs and A2L measurement/calibration files.",
    ...
    "",
    "## R06 Resume Integration",
    "",
    "- Removed specific project names, supplier names, file names, archive paths, and revision/file identifiers from the resume wording.",
    "- Preserved the engineering value: hybrid control model ownership, CAN/J1939, BMS charge, torque/speed, assist/regen, telematics, diagnostics, generated outputs, calibration files, and validation logs.",
    "- Kept the full two-page layout created in R05 while making the language safer for broad applications.",
    "",
...
def add_continuation_header(doc):
    ...
    set_font(p.add_run("KULDEEP SINGH | Automotive EV Software & Technical Lead | Page 2"), size=7.95, bold=True, color="555555")
...
def build_readme(out_dir: Path):
    readme = [
        "# R06 Privacy-Clean Full Two-Page Resume",
        "",
        "Fuller automotive EV software and technical lead resume that keeps project-verified Simulink/Stateflow evidence while removing specific project names, supplier names, file names, and archive identifiers from the resume text.",
        "",
        "## Files",
        f"- `{FILE_BASE}.docx`: editable Word resume",
        f"- `{FILE_BASE}.pdf`: rendered PDF resume",
        f"- `{FILE_BASE}.txt`: ATS/plain-text resume",
        "- `R06_Privacy_Clean_Project_Notes.md`: high-level privacy-clean project notes",
        "",
        "## Why R06 Exists",
        "- R05 proved the project evidence but included specific identifiers in the resume.",
        "- R06 removes specific project codes and supplier/project identifiers while preserving the technical proof.",
        ...
        "- Proprietary implementation details, source code, file names, and project identifiers are not copied into the resume.",
        "",
        f"Version: `{VERSION}`",
        "",
    ]
    (out_dir / "README.md").write_text("\n".join(readme), encoding="utf-8")
    (out_dir / "R06_Privacy_Clean_Project_Notes.md").write_text("\n".join(PROJECT_SCAN_NOTES), encoding="utf-8")


def main():
    for path in [OUTPUT_ROOT, REPO_R06, LOCAL_ROOT]:
        clean_dir(path)
    ...
    for file in OUTPUT_ROOT.iterdir():
        if file.is_file():
            shutil.copy2(file, REPO_R06 / file.name)
